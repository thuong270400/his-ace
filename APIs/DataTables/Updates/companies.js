...
module.exports = function (req, res) {
  ...
  if (req.query && req.query.company && req.query.company_contact && req.query.company_id && req.query.company_contact_id) {
    // Câu lệnh truy vấn
    console.log(req.query.company);
    console.log(req.query.company_contact);
    ...
    const insert = gql`
      mutation MyMutation($_set: his_ace_companies_set_input = ${CompanyStr}, $_set1: his_ace_company_contacts_set_input = ${CompanyContactStr}) {
        update_his_ace_companies(where: {id: {_eq: ${req.query.company_id}}}, _set: $_set) {
          affected_rows
          returning {
            id
          }
        }
        update_his_ace_company_contacts(where: {id: {_eq: "${req.query.company_contact_id}"}}, _set: $_set1) {
          affected_rows
          returning {
            id
          }
