<template>
  ...
  <v-card-title v-if="!circle_loader.state">
    <v-row>
      <v-col style="font-size: 15px"> DANH SÁCH GÓI KHÁM</v-col>
    </v-row>
    <v-row>
      <v-col>
        <v-text-field
    ...
    <template v-slot:item.schedules="{ item }">
      <div style="white-space: pre-wrap">
        ...
        >
          ...
          >&nbsp;-&nbsp;
          <span style="color: #1867c0"
            >buổi {{ item2.appointment_session.name }}</span
          >&nbsp;-&nbsp;
          <span style="color: #1867c0">số lượng {{ item2.total_slot }}</span
          ><br />
        </span>
      </div>
    </template>
      ...
      <tr>
        <td :colspan="columns.length">
          <v-card class="mx-auto ma-2" style="background-color: #cedee7">
            <PatientsOfPack :pack_id="item.id" :is_follow_pack="true" />
          </v-card>
        </td>
      </tr>
    </template>
